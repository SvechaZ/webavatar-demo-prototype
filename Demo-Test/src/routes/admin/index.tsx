...
// ── Mock Orders Generator for Demo ──
const generateMockOrders = () => {
  const mockOrders: any[] = [];
  const now = new Date();
  
  const menuItems = [
    { name: "ข้าวกะเพราเนื้อโคขุนสับไข่ดาว", price: 95 },
    { name: "ข้าวผัดปูจักรพรรดิ", price: 120 },
    { name: "ผัดไทยกุ้งแม่น้ำซอสส้มมะขาม", price: 150 },
    { name: "แกงเขียวหวานเนื้อริบอายเส้นโรตี", price: 185 },
    { name: "ข้าวเหนียวมะม่วงน้ำดอกไม้", price: 85 },
    { name: "ต้มยำกุ้งแม่น้ำน้ำข้นเสิร์ฟหม้อไฟ", price: 220 },
    { name: "บัวลอยสามสีลูกชิดมะพร้าวอ่อน", price: 65 }
  ];

  const orderTypes = ["dine-in", "delivery", "takeaway"];
  const customerNames = ["คุณกิตติศักดิ์", "คุณรัตนาพร", "คุณพงษ์ภัทร", "คุณนลินี", "คุณวรวุฒิ", "คุณศิริเพ็ญ", "คุณธนพล", "คุณอมรรัตน์", "คุณณัฐพงศ์", "คุณประภาส"];
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    // 3-5 orders on weekends, 2-3 on weekdays
    const numOrders = isWeekend ? (Math.floor(Math.random() * 3) + 3) : (Math.floor(Math.random() * 2) + 2);
    
    for (let j = 0; j < numOrders; j++) {
      const specificDate = new Date(orderDate);
      specificDate.setHours(10 + Math.floor(Math.random() * 11), Math.floor(Math.random() * 60), 0, 0);
      ...
      mockOrders.push({
        ...
        orderNumber: `#AK-${orderSeq}`,
        date: specificDate.toLocaleDateString("th-TH", { day: "numeric", month: "short", year: "numeric" }) + " · " + specificDate.toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit" }),
        items: items,
        subtotal: subtotal,
        ...
        tableNumber: orderType === "dine-in" ? String(Math.floor(Math.random() * 8) + 1) : "",
        queueNumber: orderType !== "dine-in" ? `Q${Math.floor(Math.random() * 50) + 10}` : "",
        note: "",
  ...
  return mockOrders.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
};
...
function AdminDashboard() {
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        if (localStorage.getItem("ran-lung-get-admin-demo") === "true") {
          setAdminUser({
            id: "demo-admin-id",
  ...
  const fetchSupabaseOrders = async () => {
    ...
    try {
      ...
      if (!error && dbOrders) {
        ...
        if (mapped.length === 0) {
          console.log("[Demo] Database orders table is empty. Injecting realistic mock orders...");
          setOrders(generateMockOrders());
        } else {
          setOrders(mapped);
        }
      } else {
        console.warn("[Demo] Failed to fetch database orders, falling back to mock orders...");
        setOrders(generateMockOrders());
      }
    } catch (e) {
